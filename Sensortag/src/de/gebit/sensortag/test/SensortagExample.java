 ...
 * 
 */
package de.gebit.sensortag.test;
...
//				System.out.println("UUID: " + service.getUUID());
	 ...
	 * characteristic exposed by the device over Bluetooth Low Energy. The
	 * parameter provided to the program should be the MAC address of the
	 * device.
	 *
	...
	public static void main(String[] args) throws InterruptedException, ClassNotFoundException {
		 ...
		 * BluetoothManager. There can be only one BluetoothManager at one time,
		 * and the reference to it is obtained through the getBluetoothManager
		 * method.
		 */
		BluetoothManager manager = BluetoothManager.getBluetoothManager();
		...
		BluetoothDevice sensor = getDevice("A0:E6:F8:B6:4D:86");
		...
		/*
		 * Each second read the value characteristic and display it in a human
		 * readable format.
		 */
		while (running) {
			 ...
			 * format to celsius and print it. Conversion for object temperature
			 * depends on ambient according to wiki, but assume result is good
			 * enough for our purposes without conversion.
			 */
